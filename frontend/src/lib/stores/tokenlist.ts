...
import uniswapDefaultList from '../../../public/tokenlists/uniswap-default.json';
...
export async function initTokenlists(): Promise<void> {
  ...
  // 1. Bundled Uniswap default tokenlist (no fetch needed)
  const defaultList: TokenlistEntry = {
    name: uniswapDefaultList.name,
    tokens: uniswapDefaultList.tokens as TokenlistToken[],
    isDefault: true,
    enabled: states['default'] !== false, // default on unless explicitly disabled
    timestamp: uniswapDefaultList.timestamp,
    version: uniswapDefaultList.version,
  };
